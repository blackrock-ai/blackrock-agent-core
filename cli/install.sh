...
if [[ -f "$TARGET_REPO/bun.lock" ]]; then
  INSTALL_CMD=(bun add "${pkgs[@]}")
elif [[ -f "$TARGET_REPO/pnpm-lock.yaml" ]]; then
...
if [[ "$DRY_RUN" == "true" ]]; then
  echo "[dry-run] cd '$TARGET_REPO' && ${INSTALL_CMD[*]}"
else
  (
    cd "$TARGET_REPO"
    "${INSTALL_CMD[@]}"
...
PY
  ...
  if [[ -z "$pending_sources" ]]; then
    ...
    SUPABASE_PROJECT_REF="$SUPABASE_PROJECT_REF" bun "$SCRIPT_DIR/prepare-migrations.ts" "${PM_ARGS[@]}"
...
FUNCTION_PATH="$TARGET_REPO/supabase/functions/agent/index.ts"
FUNCTION_CONTENT='// Auto-generated by Agent Core installer. Do not edit by hand.
import { createAgentHandler } from "npm:@blackrock-ai/agent-runtime";
